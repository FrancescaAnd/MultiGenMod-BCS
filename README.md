...
It is implemented a generative framework for **multi-view mammogram synthesis** that leverages **deep learning techniques** to enhance breast cancer screening and analysis. The system is trained on the **CBIS-DDSM** and **INbreast** datasets, which provide **high-resolution mammograms** along with **radiomic features**, making them ideal for multi-view generation. The pipeline includes a **pre-processing stage** to normalize and standardize mammogram inputs and radiomic vectors. A **UNet-based generator** conditioned on **radiomic features** is used to synthesize complementary mammogram views (e.g., generating **MLO from CC**), while a **PatchGAN discriminator** ensures **realistic image fidelity**. Additionally, a **radiomics-conditioned synthesis module** is exploited to generate mammogram patches consistent with **lesion-specific radiomic profiles**. This approach as the purpose of allowing for **richer data representations** that can support **multi-view training**, **data augmentation**, and **interpretability** in breast cancer screening workflows. As future improvements, should be explored both integration of the generative outputs with **diagnostic models** to further enhance **CAD systems** and the application of **diffusion models** could be explored (on the CBIS-DDSM dataset or an augmented version) to potentially achieve higher fidelity and diversity in multi-view generation.
